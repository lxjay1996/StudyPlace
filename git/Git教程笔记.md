...
- 自己拥有Foke后的仓库的读服务器写权限；
- 可以推送`pull request`给官方仓库来贡献代码。

## 七.自定义Git
在Git中我们不仅可以配置`user.name`和`user.email`，还可以配置很多选项，比如让Git显示颜色，会让命令输出看起来更加醒目：`git config --global color.ui true`.
### 1.忽略特殊文件
- 忽略某些文件时，需要编写`.gitignore`;
>在 Git 工作区的根目录下创建一个特殊的.gitignore文件，然后把要忽略的文件名填进去，Git 就会自动忽略这些文件。
>忽略文件的原则是：
>- 忽略操作系统自动生成的文件，比如缩略图等；
>- 忽略编译生成的中间文件、可执行文件等，也就是如果一个文件是通过另一个文件自动生成的，那自动生成的文件就没必要放进版本库，比如 Java 编译产生的.class文件；
>- 忽略你自己的带有敏感信息的配置文件，比如存放口令的配置文件。
- `.gitignore`文本本身要放到版本库里，并且可以对`.gitignore`做版本管理。

### 2.配置别名---偷懒
- 针对一些较长较难记住的命令，我们可以给命令取上别名，通过别名来调用命令。
>```linux
>	git config --global alias.st status #st是status的别名
>	git config --global alias.ci commit #ci是commit的别名
>	git config --global alias.br b使用ranch #br是branch的别名
>	git config --global alias.unstage 'reset HEAD' #unstage是`reset HEAD`的别名
>```
>之后，提交就可以简写为`git ci -m "blabla..."`
>`--global`参数是全局参数，也就是这些命令在这台电脑的所有 Git 仓库下都有用。
>加上`--global`是针对当前用户起作用的，如果不加，那只针对当前的仓库起作用。
- 每个仓库的Git配置文件都在`.git/config`文件中，别名就在[alias]后面，要删除别名，直接把对应的行删掉即可。
- 而当前用户的 Git 配置文件放在用户主目录下的一个隐藏文件`.gitconfig`中，配置别名也可以直接修改这个文件，如果改错了，可以删掉文件重新通过命令配置。

### 3.搭建Git服务器
Github就是一个免费托管开源代码的远程仓库。但是有些公司既不想公开源代码，又舍不得给Github交保护费，那就只能自己搭建一台Git服务器作为私有仓库使用。

## 八.使用SourceTree--git的图形界面工具之一
